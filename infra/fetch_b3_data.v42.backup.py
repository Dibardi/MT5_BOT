# ==============================================
# MT5_BOT — fetch_b3_data.py (versión 4.0 — datetime fix + validación de rango)
...
def parse_dates_safe(start, end, fallback_start="2018-01-01"):
    try:
        ...
        start = datetime.strptime(fallback_start, "%Y-%m-%d")
        end = datetime.now()
    return start, end
        ...
        try:
            ...
            if not df.empty:
                ...
                if first_date < "2000-01-01":
                    ...
                    status = "✅ OK"
                filename = f"{ticker.replace('.', '_')}_{interval}_{start_dt.strftime('%Y-%m-%d')}_{end_dt.strftime('%Y-%m-%d')}.csv"
