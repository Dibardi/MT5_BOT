# ml/signals/signal_generator_fix_final.py
"""
Signal Generator FIX FINAL - ML v1.2.6+
- Preserves Ticker correctly from infra/processed/merged_data.csv
- Supports selecting a single ticker (--ticker PETR4), multiple tickers (comma-separated),
  or ALL tickers with --ticker ALL (default).
- Supports model selection (--model v1_2 or v1_3)
- Supports appending to a master file (--append)
Usage examples:
    python -m ml.signals.signal_generator_fix_final --model v1_3 --ticker ALL --append
    python -m ml.signals.signal_generator_fix_final --model v1_2 --ticker PETR4 --append
"""
...
def generate_for_tickers(model, merged_df: pd.DataFrame, tickers_list, model_version:str, do_append:bool=False):
    ...
    for t in tickers_list:
        ...
        print(f"[SIGNALS_FIX_FINAL] Signals saved to: {output_file} (rows: {len(out)})")
        # append to master if requested (deduplicate by Date+Ticker)
        if do_append:
            ...
            print(f"[SIGNALS_FIX_FINAL] Appended to master: {MASTER_FILE}")
        # log for this ticker
        log = {"model_used": model_version, "ticker": t, "rows": int(len(out)), "output_file": str(output_file), "appended": bool(do_append)}
        with open(LOGS_DIR / f"signal_generation_fix_final_{t}_{ts}.json", "w", encoding="utf-8") as f:
...
def main(model_version: str, ticker_arg: str, do_append: bool):
    ...
    if 'Ticker' not in merged.columns:
        raise RuntimeError("Merged data does not contain 'Ticker' column. Please regenerate merged_data.csv with Ticker included.")
    ...
    print(f"[SIGNALS_FIX_FINAL] Model: {model_path.name} | Tickers: {len(tickers_list)} | Append: {do_append}")
    results = generate_for_tickers(model, merged, tickers_list, model_version, do_append)
    print("[SIGNALS_FIX_FINAL] Completed. Files generated:", results)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", type=str, default="v1_3", help="v1_2 or v1_3")
    parser.add_argument("--ticker", type=str, default="ALL", help="Ticker or comma-separated list of tickers (default ALL)")
